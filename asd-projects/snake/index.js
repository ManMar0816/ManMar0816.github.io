...
function init() {
  ...
  updateInterval = setInterval(update, 150); // update the game every 150 ms
}
...
function moveSnake() {
  ...
  */
  /* start, stop, and update statements to loop backwards through the indexes of snake.body */
  ...
  */
  ...
  repositionSquare(snake.head); // update the head's position on the screen
}
    ...
    */
    ...
    if (snake && Array.isArray(snake.body)) {
      for (var i = 0; i < snake.body.length; i++) {
        var part = snake.body[i];
        if (
          part.row === randomPosition.row &&
          part.column === randomPosition.column
        ) {
